...
## Mission control (общий мозг my-manager)

Этот репозиторий координируется через my-manager API. Ты — ИСПОЛНИТЕЛЬ проекта `direct-port`.
Перед работой и по ходу читай/пиши общий контекст и задачи (HTTP, заголовок `X-Internal-Key`).
Полный протокол: `my-manager/docs/CONTEXT_PROTOCOL.md`.

Окружение (ключ — секрет, не коммитить; выставь в шелле, напр. в `~/.zshrc`):
- `MM_API="https://my-manager.ioncreature.work/api"`
- `MM_KEY="<API_INTERNAL_KEY — как в GitHub-окружении production>"`

В начале сессии:
- `curl -s -H "X-Internal-Key: $MM_KEY" "$MM_API/goals/active"` — цель миссии
- `curl -s -H "X-Internal-Key: $MM_KEY" "$MM_API/tasks?project=direct-port&status=todo"` — мои задачи
- `curl -s -H "X-Internal-Key: $MM_KEY" "$MM_API/context?project=direct-port&limit=20"` — недавний контекст

Взять задачу (атомарно; `409` = уже взята, бери другую):
`curl -s -H "X-Internal-Key: $MM_KEY" -H "Content-Type: application/json" -X POST "$MM_API/tasks/<id>/claim" -d '{"contextId":"direct-port"}'`

Отчёт / решение / блокер (`blocker` будит менеджера):
`curl -s -H "X-Internal-Key: $MM_KEY" -H "Content-Type: application/json" -X POST "$MM_API/context" -d '{"author":"direct-port","project":"direct-port","kind":"note","body":"что сделал/узнал"}'`

Закрыть задачу: `curl -s -H "X-Internal-Key: $MM_KEY" -H "Content-Type: application/json" -X PATCH "$MM_API/tasks/<id>" -d '{"status":"done"}'`
