...
- Модули верхнего уровня (app.module.ts): Auth, Users, Companies, TnVed, TelegramUsers, Documents, Dashboard, Conversations, CalculationConfig, AiConfig, Countries, Regulatory, BotLinks, Bots, TopUp, ClientPortal, PublicCalculator (+ глобальный CryptoModule — шифрование секретов)
  - Auth, Users — авторизация и управление пользователями. При логине — **тенант-гейт**: если запрос несёт домен (заголовок `x-tenant-host`, проставляется прокси admin-web), пользователь не-super_admin должен принадлежать компании этого домена (иначе 401, сообщение неотличимо от неверного пароля); super_admin входит с любого домена; без домена гейт не применяется (изоляция всё равно на company-scope). Домен резолвится через `CompaniesService.resolveCompanyIdByDomain` (неизвестный домен → дефолтная компания)
  ...
  - Documents — загрузка (Telegram + админка), обработка, переобработка, скачивание, token-stats. Зонтичный модуль: агрегирует внутри AiParser/Classifier/Calculator/DutyInterpreter/Currency/CalculationLogs/Tks, они не импортируются на верхнем уровне. Здесь же `ClientCodeCatalogService` — каталог подтверждённых кодов ТН ВЭД клиента (`client_product_codes`): ключ sha256(описание+артикул) на пару (companyId, telegramUserId); processor ищет хиты ДО классификатора (хит → ставки из TKS, `codeSource='catalog'`, минует AI/vision), после успешного прогона пишет exact-строки (source='auto'), ручной выбор кода оператором — source='manual' (не перезаписывается auto). Все операции каталога best-effort — сбой не роняет pipeline
  - Dashboard — сводная статистика админ-дашборда: `GET /dashboard/stats?period=week|month|year&companyId=` (ADMIN+CUSTOMS). Окно — последние 7/30/365 дней по `created_at`. Отдаёт: документы по статусам, позиции (всего/успешно посчитанные/сумма таможенных платежей ₽ из resultData), качество классификации (`quality`: строки exact/review по `verificationStatus`, хиты каталога клиента и ручные коды по `codeSource`, средняя `matchConfidence` — из того же LATERAL-прохода по resultData), клиенты (всего/новые/активные), биллинг (суммарный баланс депозитов — скоуп напрямую по `billing_accounts.company_id`, пополнения topup, чистые списания charge−возвраты, pending-заявки), series по дням (год — по месяцам, UTC), последние документы. `companyId` — только super_admin (resolveCompanyScope); `users`/`ai` (AI-токены за период, `DocumentsService.getAiTokensSummary`) — null для роли customs
  ...
  - ClientPortal — client-scoped internal-API личного кабинета (потребитель — client-bff, auth ТОЛЬКО по X-Internal-Key, `@Internal()`). Неймспейс `/internal/client/*`: `GET /company?slug=` (публичная инфа компании по slug для рендера виджета входа; нет slug → дефолтная — `TelegramVerifyService`), `POST /verify-telegram` (верификация подписи Telegram Login Widget токеном client-бота компании из БД по slug: `SecretCipher` расшифровывает токен; служебная дефолтная компания без бота / невалидная подпись → 401; per-company, Ф4), `POST /resolve` (upsert клиента по паре `(companyId, telegramId)` → `{telegramUserId, billingAccountId, companyId}`, обёртка над `TelegramUsersService.register`), `GET /:accountId/balance`, `GET /:accountId/transactions`, `GET /:accountId/documents`, `GET /:accountId/documents/:id`, `GET /:accountId/documents/:id/download` (Excel, только PROCESSED). Ф2 (пополнение): `GET /packages`, `POST /:accountId/topups` (создать заявку, фикс цены сервером), `GET /:accountId/topups`, `POST /:accountId/topups/:id/cancel`. Ф3 (self-service): `POST /:accountId/documents` (multipart .xlsx/.csv → `DocumentsService.createFromFile` source='self_service' autoStart → штатный pipeline; сверяет принадлежность `telegramUserId` аккаунту, как `createTopUp`; rate-limit на аккаунт — fixed window в Redis, `CABINET_UPLOAD_HOURLY_LIMIT` в час, дефолт 10, превышение → 429 `UPLOAD_RATE_LIMITED`, недоступный Redis лимит не применяет). `accountId` приходит из проверенного client-JWT; документы/заявки скоупятся по `billing_account_id`, чужой ресурс → 404. Переиспользует `ClientBalanceService`, `TopUpService`, `DocumentsService`, `ExcelExportService` (экспортированы из `DocumentsModule`). См. `docs/CLIENT_CABINET.md`
  - TopUp — денежный слой кабинета (Ф2). `TopUpService`: создание заявки на пополнение (`TopUpRequest`, цена фиксируется из пакета `top-up/packages.ts`, уведомление менеджеру через `manager-notifications`), список/отмена клиентом, подтверждение/отклонение менеджером. Зачисление кредитов — `ClientBalanceService.confirmTopUp` (под локом аккаунта, идемпотентно по `DepositTransaction.sourceRequestId`; единый writer баланса сохранён). `ManagerTopUpController` (`@Internal`, `/manager/topups/:id/confirm|cancel`) — для manager-bot (резолв менеджера по `managerTelegramId`). Пакеты: `p100/p500/p5000` покупаемые, `free` (50 позиций) — не покупается: его размер служит дефолтом welcome-grant'а — при создании нового клиента `TelegramUsersService.register` идемпотентно начисляет приветственный бонус (`ClientBalanceService.grantWelcome`, type='grant', не входит в «пополнения» дашборда; env `WELCOME_GRANT_POSITIONS` переопределяет, 0 — выключить)
  - Companies — компании-тенанты (мультитенантность). CRUD только для глобального super_admin (`/companies`; admin/customs → 403). К компании привязаны User (FK RESTRICT) и TelegramUser/Document/BillingAccount/Lead/ConversationMessage (`company_id`). Дефолтная компания `00000000-0000-0000-0000-000000000001` (миграция AddMultiTenancy). Tenant-скоуп — `resolveCompanyScope`/`assertSameCompany` (`common/tenant/actor-context`). У компании есть **домены** (`CompanyDomain`, 0..N, глобально уникальны) и **тема** оформления админки (`theme`, `default`|`sky`, список — `common/tenant/company-theme.ts`). По домену запроса admin-web резолвит компанию: тему (`resolveThemeByDomain`) и гейт входа (`resolveCompanyIdByDomain`). Публичный `GET /tenant/theme?domain=` (`@Public()`, `TenantController`) отдаёт только тему тенанта (не имя/не токены) для темизации admin-web до логина. DTO create/update принимают `theme?` и `domains?` (полная реконсиляция набора; нормализация host'а — lowercase без схемы/порта/пути)
  ...
  - PublicCalculator — публичный мини-калькулятор лендинга (`POST /public/calculator/estimate`, `@Public()`, без авторизации; конвертер холодного трафика — канал № 3 `docs/PROMOTION_STRATEGIES.md`). Вход: `query` (описание или код ТН ВЭД), `price`+`currency` (USD/CNY/EUR/RUB, итог по партии), опц. `weight`/`quantity`/`code` (клик по альтернативе)/`lang`. Переиспользует `TnVedService.searchTks` (с опцией `maxEnrich` — обогащаются только топ-4 выдачи), `CalculatorService` (детерминистический fallback, без AI-интерпретатора; специфический акциз зануляется с нотой) и `CurrencyService` (суммы в ₽ по курсу ЦБ). Ответ: код+описание, `dutyRateDisplay`, суммы в ₽ (товар/пошлина/акциз/НДС/платежи/итого), `isEstimate`, notes, до 3 альтернативных кодов. Anti-abuse: fixed-window rate-limit по IP (`PUBLIC_CALC_HOURLY_LIMIT`, дефолт 20/час, 0 — выключить; IP из `cf-connecting-ip` — `common/client-ip.ts`, общий с AuthThrottleGuard); Claude дёргается максимум один раз на запрос (перевод не-русского запроса, haiku). Ошибки: 404 `CALC_NOTHING_FOUND`, 429 `CALC_RATE_LIMITED`, 503 `CBR_UNAVAILABLE`
...
- Entities: User (+ companyId, managerTelegramId), Company (тенант; + зашифрованные токены и username клиентского/менеджерского ботов, `theme`), CompanyDomain (домены тенанта — company_id + уникальный `domain`), RefreshToken, TnVedCode, TelegramUser (+ language, billingAccountId, companyId NOT NULL — клиент уникален парой `(company_id, telegram_id)`), Document (+ language, countryOfOrigin, tokenUsage, freightCost+freightCurrency, companyId), CalculationConfig, CalculationLog, TksCache, AiConfig, AiUsageLog, BillingAccount (+ companyId NOT NULL — источник правды тенант-скоупа биллинга), DepositTransaction (+ sourceRequestId), TopUpRequest, ClientProductCode (каталог подтверждённых кодов клиента), LeadSearch (+ companyId — журнал discovery per-company)
...
- **Темизация под тенанта по домену:** корневой `layout.tsx` (server, async) резолвит тему по хосту запроса через публичный `GET /tenant/theme` (`lib/tenant.ts`, сбой/неизвестный домен → дефолт) и вешает `data-theme` на `<html>`; палитра — оверрайды CSS-переменных в `globals.css` (`:root[data-theme='sky']`, светло-голубая тема). Марка бренда перекрашивается через `--brand-mark-*`. Прокси `api/[...path]` прокидывает исходный host как `x-tenant-host` для тенант-гейта логина. Компанией доменов и темы управляет super_admin на странице «Компании»
...
- Маркетинговый сайт directport.ru (App Router, без БД; единственный серверный роут — прокси мини-калькулятора). Деплоится отдельным подом: Dockerfile + Helm `landing-deployment`/`landing-service`; в локальном dev — в PM2 (`ecosystem.config.js`)
- Главная — одностраничник (Hero → тизер калькулятора → блок боли → «Кому это нужно» (`#audience`) → «Как работает»: managed-флоу пишете боту → менеджер → 10 мин → Excel → «Что получаете» → «Пример отчёта» (`#report`: три листа реального расчёта таблицами + итоги по поставке) → цена $1/позиция → «Сравнение» (`#compare`: короткая таблица + ссылка на `/compare`) → «Почему расчёту можно доверять» → «Границы применимости» (`#limits`: где нас достаточно / где нужен декларант) → «Что внутри расчёта» → гарантия → FAQ → CTA). Данные примера отчёта захардкожены в словарях (реальный расчёт, сантехника из Китая) — не подтягиваются из api. Стили — общий `globals.css` (CSS-переменные, без UI-библиотек; зависимости только next/react). Общий «хром» (Header/Footer/LangSwitch + их иконки) — `[lang]/site-chrome.tsx`, используется главной и `/calculator`; в меню хедера 4 пункта (Как работает / Что получаете / Цена / Калькулятор), навигация скрывается ≤1000px, якорные ссылки ведут на главную локали (работают и с внутренней страницы)
- **Страница `/compare`** (`[lang]/compare/page.tsx`, SSG на ru/en/zh) — сравнение с другими способами посчитать импорт: сводная таблица по 9 критериям (мини-калькуляторы / AI-подбор кода / софт декларанта / брокер / DirectPort) + 4 разбора по категориям (что делает хорошо, где упирается, чем отличаемся) + блок «как читать это сравнение» с датой сверки и ссылками на источники. Фактура и цены — `docs/COMPETITIVE_LANDSCAPE.md`; при обновлении цифр правятся оба файла. Своя SEO-мета, canonical + hreflang через `pagePath`/`hreflangAlternates('compare')`; ссылка из футера (`nav.compare`) и из секции `#compare` на главной. Заложено под дробление на отдельные страницы по категориям
- **Страница `/calculator`** (`[lang]/calculator/page.tsx`, SSG на ru/en/zh: `/calculator`, `/en/calculator`, `/zh/calculator`) — публичный мини-калькулятор: один товар без регистрации (описание/код ТН ВЭД + стоимость партии USD/CNY/EUR/RUB + опц. вес/количество → код, ставки, суммы в ₽, до 3 альтернативных кодов, предупреждения, CTA «весь прайс — в Telegram»). Своя SEO-мета (`miniCalc.page.metaTitle/metaDescription` — под транзакционные запросы «калькулятор таможенных платежей», канал № 1 стратегии), canonical + hreflang через `pagePath`/`hreflangAlternates(subPath)`. Сама форма — client component `[lang]/mini-calculator.tsx` (чистая карточка без секции-обёртки); на главной вместо формы — тизер-секция `#calculator` со ссылкой на страницу. Браузер ходит same-origin в `POST /api/calc` (`src/app/api/calc/route.ts`) — прокси к `POST /public/calculator/estimate` api (модуль PublicCalculator, rate-limit по IP на стороне api; заголовок `cf-connecting-ip` пробрасывается). Ошибки локализуются по `code` ответа (`CALC_NOTHING_FOUND`/`CALC_RATE_LIMITED`). LangSwitch принимает `subPath` — смена языка не уводит со страницы
- **Локализация (ru/en/zh)** через роутинг App Router: сегмент `src/app/[lang]/` (`layout`, `page`, `calculator/page`, `compare/page`), `generateStaticParams` → `ru|en|zh`, `dynamicParams=false` (корректный серверный `<html lang>`, `hreflang`-alternates, canonical, `og:locale`). Русский живёт на корне без префикса (`/`, `/calculator`): `src/middleware.ts` тихо rewrite'ит путь в `/ru...` (URL не меняется, без редиректа; matcher — `['/', '/calculator', '/compare']`); `/en...`, `/zh...` — с префиксом; неизвестные локали → 404. Весь копирайтинг вынесен в типизированные словари `src/app/i18n/dictionaries/{ru,en,zh}.ts` (`ru` — источник истины, тип `Dictionary`), конфиг локалей — `i18n/config.ts`. Переключатель языков (сегментированный `RU/EN/中文`) в шапке и футере. Языконезависимые константы (бренд, вордмарк, палитра, контакты) — `src/app/_brand.ts`. Китайский рендерится системными CJK-шрифтами (без вебшрифта). OG-**картинка** (`opengraph-image.tsx`) общая на ru — Satori не имеет фолбэка на системные шрифты, CJK-глифы в изображении не рендерятся (title/description HTML локализованы отдельно)
...
- ENV: `NEXT_PUBLIC_TELEGRAM_BOT_URL` (ссылка на бота, дефолт `https://t.me/direct_port_bot`), `NEXT_PUBLIC_SITE_URL` (для metadataBase/OpenGraph), `CALC_API_URL` (внутренний адрес api для прокси мини-калькулятора, дефолт `http://localhost:3001/api`). Контакты (email, телефон) захардкожены в `src/app/_brand.ts`
...
- **Боты per-company:** не один синглтон-бот, а реестр `BotRegistry` — один `Bot` на компанию, токены только из `GET /internal/bots` (env `TELEGRAM_BOT_TOKEN` для подъёма бота не используется — легаси удалён). Реестр дедуплицирует боты по `bot_id` (две компании с одним токеном не поднимут два `getUpdates` → иначе 409), а сбой polling одного бота изолирован (`bot.start().catch`) и не роняет остальные. Подписан на Redis `bot-config-events` → динамический подъём/перезапуск/остановка без передеплоя (+ реконсайл каждые 5 мин). `OutgoingHandler` выбирает `Api` по `companyId` из job (нет бота компании → доставка в failed-job). Входящий апдейт несёт `ctx.companyId` бота. См. `docs/COMPANY_BOTS.md`
...
- **Боты per-company:** реестр `BotRegistry` (один `Bot` на компанию; токены только из `GET /internal/bots`, env-подъём удалён). Дедуп по `bot_id` + изоляция сбоя polling одного бота (`bot.start().catch`). Подписан на Redis `bot-config-events` → динамический reload (+ реконсайл 5 мин). `NotifyHandler` выбирает `Api` по `companyId` из уведомления. См. `docs/COMPANY_BOTS.md`
...
→ [Воркер] Каталог кодов клиента (ClientCodeCatalogService): хиты по (companyId, telegramUserId, sha256(описание+артикул)) получают подтверждённый код без AI (codeSource='catalog'), остальные строки →
...
| GET | `/:id/download-import-sheet` | ADMIN, CUSTOMS | Плоский xlsx товарной части для импорта в декларантское ПО (Контур.Декларант «Загрузка товаров из файла»; годится для СТМ-Конвертера/Альта-Заполнителя): один лист, строка = позиция, «№ товара» группирует в товары ДТ, страна цифровым кодом |
...
- `WELCOME_GRANT_POSITIONS` — размер приветственного бонуса нового клиента в позициях (пусто — дефолт из пакета `free`, 50; 0 — выключить)
- `CABINET_UPLOAD_HOURLY_LIMIT` — лимит self-service загрузок из кабинета на аккаунт в час (пусто — 10; 0 — выключить)
- `PUBLIC_CALC_HOURLY_LIMIT` — лимит запросов публичного мини-калькулятора лендинга на IP в час (пусто — 20; 0 — выключить)
...
- `docs/COMPETITIVE_LANDSCAPE.md` — конкурентный ландшафт и прайсинг: чем пользуются декларанты (Альта/Контур/СТМ/ТКС) и почём, два ценовых якоря ($1/позиция против 3 ₽ калькулятора и 1–15 тыс. ₽ ручного подбора кода), разбор гипотезы «продукт дорогой»
- `docs/PROMOTION_STRATEGIES.md` — стратегии продвижения: диагноз холодных звонков, сегменты ЦА, каналы (авто/полуавто/ручные), план 30–60 дней, внутренние маркетинг-инструменты, профиль помощника
