...
- **Локализация (ru/en/zh)** через роутинг App Router: сегмент `src/app/[lang]/` (`layout`, `page`, `calculator/page`), `generateStaticParams` → `ru|en|zh`, `dynamicParams=false` (корректный серверный `<html lang>`, `hreflang`-alternates, canonical, `og:locale`). Русский живёт на корне без префикса (`/`, `/calculator`): `src/middleware.ts` тихо rewrite'ит путь в `/ru...` (URL не меняется, без редиректа; matcher — `['/', '/calculator']`); `/en...`, `/zh...` — с префиксом; неизвестные локали → 404. Весь копирайтинг вынесен в типизированные словари `src/app/i18n/dictionaries/{ru,en,zh}.ts` (`ru` — источник истины, тип `Dictionary`), конфиг локалей — `i18n/config.ts`. Переключатель языков (сегментированный `RU/EN/中文`) в шапке и футере. Языконезависимые константы (бренд, вордмарк, палитра, контакты) — `src/app/_brand.ts`. Китайский рендерится системными CJK-шрифтами (без вебшрифта). OG-**картинка** (`opengraph-image.tsx`) общая на ru — Satori не имеет фолбэка на системные шрифты, CJK-глифы в изображении не рендерятся (title/description HTML локализованы отдельно)
