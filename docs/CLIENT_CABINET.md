...
- **Ф2 — пополнение ✅ сделано (без онбординга):** `TopUpRequest` + пакеты (`top-up/packages.ts`)
  + создание заявки из кабинета + подтверждение/отклонение менеджером в manager-bot
...
- **Abuse self-service — частично закрыто:** rate-limit на загрузки реализован
  (fixed window в Redis на аккаунт, `CABINET_UPLOAD_HOURLY_LIMIT` в час, дефолт 10,
  превышение → 429 `UPLOAD_RATE_LIMITED`; недоступный Redis лимит не применяет — кабинет
  важнее анти-абьюза). Остаётся флаг на повторные `REQUIRES_REVIEW` от одного клиента —
  реализуем, если случится.
- **Онбординг — сделан в упрощённом виде:** welcome-grant начисляется автоматически при
  создании нового клиента (`TelegramUsersService.register` → `ClientBalanceService.grantWelcome`,
  идемпотентно, type='grant', размер — пакет `free`, env `WELCOME_GRANT_POSITIONS`
  переопределяет, 0 — выключить). Claim первого касания (привязка к лиду) — по-прежнему отложен.
