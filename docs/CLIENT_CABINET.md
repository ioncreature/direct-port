...
> **Статус: дизайн, не реализовано.** Документ фиксирует согласованную продуктовую
> рамку и архитектуру для разворачивания по фазам (см. раздел «Фазы»). Имена новых
> сущностей/эндпоинтов — предложения; имена существующего кода — фактические.
...
- **Ф0 — биллинг-фундамент (только `apps/api`, кабинета ещё нет):** `BillingAccount` (1:1) +
  миграция баланса/ledger на аккаунт + тип `grant` + `sourceRequestId` + `CHECK balance>=0`.
  Безопасно мёржится до появления кабинета. `ClientBalanceService` начинает лочить/писать по
  `billingAccountId`.
- **Ф1 — вход + витрина (read-only):** `client-bff` + Telegram Login + client-JWT;
  `client-web` экраны баланс/история/документы/скачать. Здесь обкатывается auth-контур и
  изоляция.
