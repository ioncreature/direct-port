...
/**
 * Render an `error-{code}` Fluent key with safe fallback. If the code is missing
 * or its key is not defined, returns the fallback key (Fluent leaks `{error-FOO}`
 * placeholders for unknown keys otherwise).
 */
