...
4. Если количество вычисляется из нескольких колонок (например, коробки × штук/коробку) — используй итоговое количество (количество штук, а не коробок)
5. Вес должен быть за ОДНУ единицу товара в килограммах. Внимательно смотри, что содержит колонка веса:
   - «вес за единицу / unit weight / 单净重» — бери как есть
...
11. Если в таблице есть дополнительные числовые характеристики товара (площадь, объём, длина и т.д.) — извлеки их в массив dimensions с единицами измерения. Значение dimension — ЗА ОДНУ ЕДИНИЦУ товара (Calculator умножает его на quantity), поэтому общие объёмы из таблицы сначала делятся на количество
12. Для жидкостей (напитки, соки, вода, пиво, масло и т.п., расфасованные в банки/бутылки/канистры), если в таблице нет колонки с объёмом жидкости, оцени объём одной единицы по весу и типу упаковки:
    - банка ~0.35 кг → 0.33 л; банка ~0.52 кг → 0.5 л
    - бутылка ~0.35 кг → 0.33 л; ~0.55 кг → 0.5 л; ~1.05 кг → 1 л; ~1.6 кг → 1.5 л; ~2.1 кг → 2 л
    - плотность напитков ≈ 1 кг/л, для точной оценки округляй до стандартного размера
    Клади как dimensions элемент {name:'объём (оценка по весу)', value:<литры за единицу>, unit:'l'}. Для твёрдых товаров и упаковок с неясной плотностью — НЕ угадывай, не заполняй.
    ВАЖНО: колонки «габариты коробки» (длина/ширина/высота), «объём коробки / индивидуальный объём / cubic meter / 立方 / 单件体积» и общие м³ партии — это размеры транспортной УПАКОВКИ, а не объём жидкости. Их в dimensions[unit='l'] класть НЕЛЬЗЯ. Для оценки литров используй только вес или прямую колонку объёма содержимого
...
3. Маппинг колонок (0-indexed): наименование, цена за единицу, вес, итоговое количество.
   ...
   - Количество: колонка с ИТОГОВЫМ количеством штук (если есть «кол-во коробок» + «штук в коробке» — бери «общее количество», не количество коробок).
   - Вес: укажи колонку с весом одной единицы товара И заполни weightNote:
     - 'per_unit' — за одну штуку товара
...
Поле dimensions — необязательное. Добавляй только если в таблице есть соответствующие колонки.`;
  ...
  private normalizeDimensions(raw: unknown): Dimension[] {
    ...
    for (const entry of raw) {
      ...
      if (!name || !unit || !Number.isFinite(value) || value <= 0) continue;
      result.push({ name, value: round4(value), unit });
    }
    return result;
  }
