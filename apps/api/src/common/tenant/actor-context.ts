...
/**
 * Платформенная компания «По умолчанию» (создаётся миграцией AddMultiTenancy).
 * Используется для backfill и как дефолт @Internal-контуров без actor: агентские
 * эндпоинты лидов (discover/digest/searches/report) принимают companyId параметром
 * задания и без него работают от этой компании.
 */
